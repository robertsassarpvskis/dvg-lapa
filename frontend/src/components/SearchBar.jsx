...
import CloseIC from "/src/assets/icons/close.svg";
    ...
    <div className="bottom-0">
      ...
      {isActive && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="relative w-1/2 bg-greenLight rounded-lg shadow-lg p-6">
            <input
              type="text"
              value={searchQuery}
              onChange={handleInputChange}
              className="w-full p-4 text-xl bg-greenLight rounded-lg focus:outline-none"
              placeholder="Search..."
            />
            <button
              ...
              onClick={handleClose}
            >
              <img src={CloseIC} alt="" className='mr-1 w-7 h-7 '/>
            </button>
          </div>
        </div>
      )}
